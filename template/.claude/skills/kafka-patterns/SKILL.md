...
metadata:
  ...
  related-skills: spring-boot-engineer, tdd-guide, logging-patterns, spring-boot-patterns, java-code-review
---
...
- The task is controller/service/repository layering or DTO boundaries — use `spring-boot-patterns`
- The main problem is generic logging policy, JSON logs, or MDC usage outside Kafka transport boundaries — use `logging-patterns`
...
6. **Do you need observability or correlation headers?** Keep transport headers here, but route general logging policy back to `logging-patterns`.
...
- Correlation headers are useful transport metadata, but full logging policy still belongs to `logging-patterns`.
